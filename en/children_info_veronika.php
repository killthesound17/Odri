<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">

    <link href="https://fonts.googleapis.com/css?family=Raleway:300,400|Rochester|Roboto:300,400|Rouge+Script" rel="stylesheet">
    <link rel="stylesheet" href="../src/css/normalize.css">
    <link rel="stylesheet" href="../src/css/slick.css">
    <link rel="stylesheet" href="../src/css/main.css">
    <script type="text/javascript" src="../src/js/jquery-3.2.1.min.js">
    </script>


    <title>Veronika - CHILDREN - ODRI Models Management</title>
</head>

<body>
    <?php include 'preloader.php' ?>
    <div class="container">
      <div class="lang-buttons">
        <a href="#">EN</a>
        <a href="../ru/children_info_veronika.php">RU</a>
      </div>
    </div>
  <header>
      <?php
      include 'en-header.html' ?>
  </header>

    <main>


        <div class="container">
        <section class="model-profile">
          <div class="model-info">
            <h2 class="model-info__name">Veronika</h2>
            <div class="options">
              <ul class="model-options">
                <li class="model-options__option">height</li>
                <li class="model-options__option">bust</li>
                <li class="model-options__option">waist</li>
                <li class="model-options__option">hips</li>
                <li class="model-options__option">shoes</li>
                <li class="model-options__option">hair</li>
                <li class="model-options__option">eyes</li>
                <li class="model-options__option">age</li>
              </ul>
              <ul class="option-values">
                <li class="option-values__value">128cm</li>
                <li class="option-values__value">60cm</li>
                <li class="option-values__value">52cm</li>
                <li class="option-values__value">62cm</li>
                <li class="option-values__value">32</li>
                <li class="option-values__value">brown</li>
                <li class="option-values__value">brown</li>
                <li class="option-values__value">-</li>
              </ul>
            </div>
          </div>
          <div class="model-photos">
            <div class="photos-fullsize">
              <img src="../src/img/children/Veronika/IMG_9246.jpg" alt="" class="photos-fullsize__photo">
              <img src="../src/img/children/Veronika/IMG_9255.jpg" alt="" class="photos-fullsize__photo">
              <img src="../src/img/children/Veronika/IMG_9289.jpg" alt="" class="photos-fullsize__photo">
            </div>
          </div>
          <div class=""></div>

        </section>

        </div>
    </main>
    <footer>
      <?php include 'en-footer.php' ?>

    </footer>
    <script type="text/javascript" src="../src/js/preloader.js"></script>
    <script type="text/javascript" src="../src/js/slick.min.js">

    </script>
    <script src="https://use.fontawesome.com/e059081491.js"></script>

    <script type="text/javascript" src="../src/js/main.js"></script>
    <script type="text/javascript" src="../src/js/slider.js">

    </script>
</body>

</html>
